...
float PDS_efficiency_ivr_area(float ivr_area[])
{
    ...
    for(int i=0; i<second_stage_vr_num; i++)
    {
    second_stage_vr_loss[i] = load_current[i] * load_voltage[i] / Static_Buck(1, middle_voltage, load_voltage[i], load_voltage[i]*load_current[i], ivr_area[i], 0.01, 0.8, 0) - load_current[i] * load_voltage[i];
